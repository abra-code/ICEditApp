#!/usr/bin/env python3
"""Add the selected Material Symbol as a layer to the current icon."""
...
log("=== ICEdit.materialsymbols.add.py ===")

MATERIAL_UUID = WINDOW_UUID  # this dialog's own UUID

# Get selected symbol name
symbol = os.environ.get("OMC_ACTIONUI_TABLE_2_COLUMN_1_VALUE", "").strip()
if not symbol:
    subprocess.run(
        [DIALOG_TOOL, MATERIAL_UUID, "3", "No symbol selected"],
        capture_output=True
    )
    sys.exit(0)

svg_path = f"{SCRATCH_DIR}/icedit_matsymbol_{MATERIAL_UUID}.svg"
if not os.path.isfile(svg_path):
    subprocess.run(
        [DIALOG_TOOL, MATERIAL_UUID, "3", "No SVG rendered — select a symbol first"],
        capture_output=True
    )
    sys.exit(0)
...
if not icon_path:
    subprocess.run(
        [DIALOG_TOOL, MATERIAL_UUID, "3", "No icon open in ICEdit"],
        capture_output=True
    )
    sys.exit(0)
...
if result.returncode != 0:
    subprocess.run(
        [DIALOG_TOOL, MATERIAL_UUID, "3", f"Failed: {result.stderr.strip()[:80]}"],
        capture_output=True
    )
    sys.exit(1)
...
subprocess.run(
    [DIALOG_TOOL, MATERIAL_UUID, "3", f"Added '{symbol}' as layer"],
    capture_output=True
)

log("=== ICEdit.materialsymbols.add.py done ===")
